...
{
    // Convert venus:// to venus:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("venus://"))
    {
        uri.replace(0, 12, "venus:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Venus.lnk";
}
...
{
    return GetAutostartDir() / "venus.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Venus-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  venus-qt [" + tr("command-line options") + "]                     " + "\n";
